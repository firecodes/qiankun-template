<template>
  <section class="login container mx-auto my-10 md:my-20">
    ...
    <div class="max-w-screen-xs">
      <form class="mb-6">
        <v-text-field
          v-model="state.username" :error-messages="v$.username.$errors.map((e: any) => e.$message)"
          :label="t('login.form.username.value')" class="mb-1 md:mb-2"
          :placeholder="t('login.form.username.placeholder')" variant="outlined" required
        ...
        <v-text-field
          ...
          :placeholder="t('login.form.password.placeholder')" variant="outlined" required
          @input="v$.password.$touch" @blur="v$.password.$touch"
        />
        <v-btn color="primary" block :text="t('login.button')" class="mt-2" :loading="loading" @click="handleLogin" />
      </form>
