<template>
  <div class="p-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="(user, index) in users" :key="index" class="bg-white rounded-lg shadow-md p-4">
    ...
    <!-- Add Money Modal -->
    <div v-if="showAddMoneyModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
      <div class="bg-white rounded-lg p-8">
        <h2 class="text-lg font-semibold mb-4">
          Add Money to {{ selectedUser.name }}
        </h2>
        <input v-model="amountToAdd" type="number" class="border border-gray-300 rounded-md px-4 py-2 mb-4 w-full"
          placeholder="Enter amount">
        <div class="flex justify-end">
          <button class="bg-blue-500 text-white font-semibold py-2 px-4 rounded-md shadow-md hover:bg-blue-600 mr-2"
            @click="addMoney">
            Add
          </button>
          <button class="bg-gray-300 text-gray-800 font-semibold py-2 px-4 rounded-md shadow-md hover:bg-gray-400"
